...
class PropertyController extends Controller
{
    ...
    { 
        ...
        if($search) {
            foreach($search as $searches){
                if ($searches->price <= $property->price && $searches->purpose == $property->purpose && $searches->type == $property->type && $searches->bedroom == $property->bedroom && $searches->city == $property->city){
                    $user = User::find(Auth::id());
                    Message::create([
                        'agent_id'  => $searches->agent_id,
                        'user_id'  => Auth::id(),
                        'name'      => $user->name,
                        'email'     => " ",
                        'phone'     => " ",
                        'message'   => 'Se ha cargado una propiedad que concide con la búsqueda: Precio: '.$property->price.' Condición: '.$property->purpose.' Tipo: '.$property->type.' Nro.Habitaciones: '.$property->bedroom.' Ciudad: '.$property->city
                    ]);
                } 
            }
        }
        ...
        Toastr::success('message', 'Propiedad creada con éxito.');
    ...
    {   
        ...
        Toastr::success('message', 'Propiedad actualizada con éxito.');
        return redirect()->route('agent.properties.index');
    }
    ...
    public function destroy(Property $property)
    {
        ...
        Toastr::success('message', 'Propiedad eliminada con éxito.');
        return back();
    }
